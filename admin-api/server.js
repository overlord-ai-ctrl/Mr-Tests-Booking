...
app.post('/api/debug/init-all-data', (req, res) => {
  ...
  try {
    ...
    if (!fs.existsSync(CENTRES_PATH)) {
      ...
      if (fs.existsSync(localCentres)) {
        ...
        const defaultCentres = {
          "centres": [
            { "id": "1", "name": "Birmingham (Garretts Green)", "value": "birmingham-garretts-green" },
            { "id": "2", "name": "Birmingham (Kings Heath)", "value": "birmingham-kings-heath" },
            { "id": "3", "name": "Coventry", "value": "coventry" },
            { "id": "4", "name": "Dudley", "value": "dudley" },
            { "id": "5", "name": "Walsall", "value": "walsall" }
          ],
          "updated_at": Date.now()
        };
    ...
    res.json({ ok: true, message: 'All data files initialized', results });
  } catch (error) {
